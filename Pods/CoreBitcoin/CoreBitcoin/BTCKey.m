...
- (NSData*)signatureForHash:(NSData*)hash {
    return [self signatureForHash:hash appendHashType:NO hashType:0 kValue: nil];
}
...
- (NSData*)signatureForHash:(NSData*)hash hashType:(BTCSignatureHashType)hashType {
    return [self signatureForHash:hash appendHashType:YES hashType:hashType kValue: nil];
}
...
-(NSData*)signatureForHash:(NSData *)hash kValue: (NSMutableData*)k
{
    return [self signatureForHash:hash appendHashType:NO hashType:0 kValue:k];
}

- (NSData*)signatureForHash:(NSData*)hash appendHashType:(BOOL)appendHashType hashType:(BTCSignatureHashType)hashType kValue: (NSMutableData*)kCustom{
    CHECK_IF_CLEARED;
    ...
    if (kCustom == nil)
    {
        kdata = [self signatureNonceForHash:hash];
    }
    else
    {
        kdata = kCustom;
    ...
    //BTCMutableBigNumber* k = [[BTCMutableBigNumber alloc]initWithDecimalString:@"38100951718892604526703186896503501730592975880940954198905123114543441169593"];
