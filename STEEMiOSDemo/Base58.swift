...
func encodeAlphabet(byteSlice: [UInt8], alphabet: String) -> String {
    ...
    while cmp(bytesAsIntBig, bigZero) > 0 {
        let mod = IntBig()
        let (quotient, modulus) = divMod(bytesAsIntBig, bigRadix, mod)
        
        bytesAsIntBig = quotient
        
        // Make the String into an array of characters.
        answer.insert(byteAlphabet[getInt64(modulus)!], atIndex: 0)
    }
